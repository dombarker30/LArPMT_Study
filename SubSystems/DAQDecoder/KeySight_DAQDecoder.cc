...
int run(std::string filename, std::vector<int> chans, bool draw, bool fVerbose, int num_events)
{
  ...
  while (!filein.eof()){
	...
	//int numavg = 1;
	int iterations = 2;
	float maximum = -9999; // highest point in waveform
	float minimum = 9999;  // lowest point in waveform
	float maxpeaktime = -9999; // tick of maximum
	float minpeaktime = -9999; // tick of minimum
	float max_ped = -9999; // highest point in waveform outside integration window
	float min_ped = 9999;  // lowest point in waveform outside integration window
	...
	pedhist_map2[chans[ichan]] = new TH1I(pedhist2_name,"Reduced Pedestal Histogram",2048+1,-1024-0.5,1024+0.5);

	//Iterative Loop to calculate the integration size proper size.
	for(int iter=0; iter<iterations;++iter){

	  //std::cout<<"Iteration: "<<iter<<std::endl;
	  // reset the integration counter
	  intcounter = 0;
	  int_tot = 0;
	  ...
	  for (size_t v = 0; v <wf.size(); ++v){
	    ...
	    //calculate ped outside integration window
	    if(v < int_start || v > integration_cut){
	  ...
	  if (draw){
	    ...
	    pedhist_map1[chans[ichan]]->GetXaxis()->SetRangeUser(min_ped-1,max_ped+1);
	    pedhist_map1[chans[ichan]]->Draw();
	  }

	  int ped_mode_bin       = pedhist_map1[chans[ichan]]->GetMaximumBin();
	  float ped_entries      = pedhist_map1[chans[ichan]]->GetEntries();
	  float ped_RMS          = pedhist_map1[chans[ichan]]->GetRMS();
	  float ped_mode         = pedhist_map1[chans[ichan]]->GetXaxis()->GetBinCenter(ped_mode_bin);
	  float ped_mode_entries = pedhist_map1[chans[ichan]]->GetBinContent(ped_mode_bin);

	  // Set the max value of the mean of the gaussian:
	  // Either as the mode+RMS or max_ped (whichever is lower)
	  float pedMeanMax = ped_mode+ped_RMS;
	  if (pedMeanMax>max_ped){
	    pedMeanMax=max_ped;
	  }

	  //std::cout<<"Ped1: "<<ped_entries<<std::endl;

	  //Fit a Gaussian to the pedestal
	  TF1 *ped_fit = new TF1("ped_fit","gaus");
	  //ped_fit->SetParameter(0,ped_entries/2.);
	  ped_fit->SetParameter(0,ped_mode_entries);
	  ped_fit->SetParameter(1,ped_mode);
	  ped_fit->SetParameter(2,ped_RMS);
	  ped_fit->SetParLimits(0,0.75*ped_mode_entries,1.25*ped_mode_entries);
	  ped_fit->SetParLimits(1,ped_mode-ped_RMS,pedMeanMax);
	  ped_fit->SetParLimits(2,0.2,2.0*ped_RMS);
	  ...
	  //std::cout<<"PeakHeight: "<<ped_fit->GetParameter(0)<<"Mean: "<< ped_fit_mean <<"and std. dev."<<ped_fit_stddev<<" and ped rms: "<<ped_RMS<<" pedMax: "<<pedMax<<std::endl;

	  //Fill only values
	  // into the second pedestal fit that fall within the std dev limits
	  int minPed2 = 9999;
	  int maxPed2 =-9999;
	  for (size_t v=0; v<int_start; ++v){
	    if(TMath::Abs(wf[v] - ped_fit_mean) < (3*ped_fit_stddev)){
	      pedhist_map2[chans[ichan]]->Fill(wf[v]);
	      if(wf[v]>maxPed2){maxPed2=wf[v];}
	      if(wf[v]<minPed2){minPed2=wf[v];}
	    }
	  }
	  ...
	  if(draw){
	    ...
	    pedhist_map2[chans[ichan]]->GetXaxis()->SetRangeUser(minPed2-1,maxPed2+1);
	  ...
	  if (pedhist_map2[chans[ichan]]->GetEntries()==0) {
	    ...
	    //std::cout<<"Low limit: "<<min_ped<<" high limit "<<max_ped<<std::endl;
	    baseAdc = ped_mode;
	    baseRmsAdc = ped_RMS;

	  }else{
	    //std::cout<<"Ped2: "<<pedhist_map2[chans[ichan]]->GetEntries()<<std::endl;
	    //Fit the second pedestal graph
	    int ped2Entries = pedhist_map2[chans[ichan]]->GetEntries();

	    TF1 *ped_fit2 = new TF1("ped_fit2","gaus");
	    ped_fit2->SetParameter(0,ped2Entries/2);
	    ped_fit2->SetParameter(1,ped_fit_mean);
	    ped_fit2->SetParameter(2,ped_fit_stddev);
	    ped_fit2->SetParLimits(0,0,ped2Entries);
	    ped_fit2->SetParLimits(1,0,2*ped_fit_mean);
	    ped_fit2->SetParLimits(2,0,2*ped_fit_stddev);
	    ...
	    baseRmsAdc = ped_fit2->GetParameter(2);
	  ...
	  if((iter+1) != iterations){
	    ...
	    pedhist_map1[chans[ichan]]->Reset();
	    pedhist_map2[chans[ichan]]->Reset();
	    ...
	    if(int_start < 100){int_start = 100;}
	    ...
	    if(integration_cut > (wf.size()-100)){integration_cut = wf.size()-100;}

	    //std::cout<<"Int_Start: "<<int_start<<" and intergration cut: "<<integration_cut<<std::endl;
